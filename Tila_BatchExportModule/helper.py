...
def construct_file_path(self, output_dir, layer_name, ext, increment):
    ...
    layer_name = renamer.construct_filename(
        self, layer_name, self.filenamePattern, self.filename, ext, increment)
...
def copy_arr_to_temporary_scene(self, arr, ctype=None):
    ...
    try:
            ...
            if item_have_deformers(item):
                ...
                for gen in t.genericName:
                    for d in self.deformer_item_dict[item.name][0].deformers:
                        if gen in d.name:
                            replace_generic_deformer_name_dict(
                                d, genericDeformerName, gen)
        ...
        for i in xrange(len(modified_selection_name_arr)):
            ...
            if item_have_deformers(modo.Item(curr_item)):
                ...
                if o.name in self.replicator_group_source.keys():
                    ...
                    for source in self.replicator_group_source[o.name][1]:
                        if group_name not in replicator_group_source_ignored.keys():
                            ...
                            replicator_group_source_ignored[group_name] = [
                                source]
                        else:
                            if source not in replicator_group_source_ignored[group_name]:
                                self.scn.select(source.name, add=True)
                                replicator_group_source_ignored[group_name].append(
                                    source)
...
def return_exception():
    lx.out('Exception "{}" on line {} in file {} in scene {}'.format(sys.exc_value,
                                                                     sys.exc_traceback.tb_lineno, os.path.basename(__file__), modo.Scene().name))
    sys.exit()
...
def check_selection_count(self):
    if self.userSelectionCount == 0:  # No file Selected
        if self.exportVisible_sw:
            dialog.init_message('error', 'No item visible',
                                'At least one item has to be visible')
        else:
            dialog.init_message('error', 'No item selected',
...
def construct_replicator_dict(self):
    if len(self.itemToProceed['REPLICATOR']) > 0:
        self.replicator_dict = get_replicator_source(
            self, self.itemToProceed['REPLICATOR'])
...
def revert_scene_preferences(self):
    ...
    if self.exportFormatFbx_sw:
        lx.eval('user.value sceneio.fbx.save.exportType %s' %
                self.defaultExportSettings['FBX_EXPORT_TYPE'])
        lx.eval('user.value sceneio.fbx.save.surfaceRefining %s' %
                self.defaultExportSettings['FBX_SURFACE_REFINING'])
        lx.eval('user.value sceneio.fbx.save.format %s' %
                self.defaultExportSettings['FBX_FORMAT'])
...
def reset_import_settings(self):
    ...
    lx.eval('user.value sceneio.obj.import.static %s' %
            self.defaultImportSettings['OBJ_STATIC'])
    lx.eval('user.value sceneio.obj.import.separate.meshes %s' %
            self.defaultImportSettings['OBJ_SEPARATE_MESH'])
    lx.eval('user.value sceneio.obj.import.suppress.dialog %s' %
            self.defaultImportSettings['OBJ_SUPRESS_DIALOG'])
    lx.eval('user.value sceneio.obj.import.units %s' %
            self.defaultImportSettings['OBJ_UNIT'])
...
class ModoDeformerItem():
    def __init__(self, item):
        ...
        if not self.is_deformer_item:
            dialog.print_log(
                'The item {} has no deformer'.format(self.item_name))
